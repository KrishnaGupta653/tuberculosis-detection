...
# ──────────────────────────────────────────────────────────────────────────────
# Dataset class for DataLoader (must be top-level to be picklable on Windows)
# ──────────────────────────────────────────────────────────────────────────────

class _InferDS(Dataset):
    """GPU inference dataset. Module-level so it's picklable for multiprocessing."""
...
# ══════════════════════════════════════════════════════════════════════════════
# 4.  MAIN ENTRY POINT  — fast parallel extraction
# ══════════════════════════════════════════════════════════════════════════════
...
def extract_dataset_features(
    ...
    """
    Two-stage parallel feature extraction.

    Stage 1 (CPU, multiprocess)  — segmentation + GLCM for every image
    Stage 2 (GPU, large batches) — DenseNet inference on validated images

    Parameters
    ----------
    data_dir   : root folder with Normal/ and TB/ sub-folders
    n_jobs     : CPU workers for Stage 1  (default: NUM_CPU_WORKERS = min(cores,8))
    cache_path : .npz path; load on hit, save on miss

    Returns
    -------
    X : float32 ndarray (n_samples, 1048)
    ...
    with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as pool:
        for done, result in enumerate(
            ...
            if done % 500 == 0 or done == len(paths):
                logger.info(f'  Stage 1: [{done}/{len(paths)}]  '
                            f'failed={failed}')
    ...
    # ── Stage 2: batched GPU ──────────────────────────────────────────────────
    logger.info(f'Stage 2/2 — GPU DenseNet inference  '
                f'({n_valid} images, batch={GPU_BATCH_SIZE})…')
    ...
    if cache_path:
        ...
        logger.info(f'Features cached → {cache_path}  '
                    f'(future runs will load instantly)')
