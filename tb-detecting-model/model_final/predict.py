...
def predict_image(image_path: str, predict_fn, model_key: str) -> dict:
    """
    ...
    model_key : str — model name (svm, rf, gb, ensemble)
    ...
    try:
        ...
        # Feature slicing is handled inside predict_fn for each model
...
Examples:
  ...
  python predict.py --model ensemble --folder test_images/ --save_csv out.csv
    ...
    # ── Single image ──────────────────────────────────────────────────────────
    if args.image:
        r = predict_image(args.image, predict_fn, args.model)
    ...
    if args.folder:
        ...
        for img_path in files:
            r = predict_image(img_path, predict_fn, args.model)
            _print_result(r, args.model)
            all_results.append(r)
