...
class EnsembleModel:
    ...
    def _proba_single_pipeline(self, pipeline_idx: int, X: np.ndarray) -> np.ndarray:
        ...
        if scaler is not None:  # RF doesn't use scaling
...
def train(data_dir: str, cache_path: str | None = None):
    ...
    for i, name in enumerate(ensemble._names):
        ...
        def make_predict(m, sc, sel):
            def _predict(X):
                if sc is not None:
                    return m.predict(sc.transform(sel.transform(X)))
                else:
                    return m.predict(sel.transform(X))
            return _predict
        
        def make_proba(m, sc, sel):
            def _proba(X):
                if sc is not None:
                    return m.predict_proba(sc.transform(sel.transform(X)))
                else:
                    return m.predict_proba(sel.transform(X))
            return _proba
        
        pfn = make_predict(model, scaler, selector)
        probfn = make_proba(model, scaler, selector)
