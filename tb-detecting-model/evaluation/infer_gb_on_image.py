"""
================================================================================
INFER GRADIENT BOOSTING ON SINGLE IMAGE
================================================================================
Usage: python infer_gb_on_image.py --image path/to/image.png
"""
...
def infer_gb_on_single_image(image_path, model_path=None, verbose=True):
    """Predict TB on single image using Gradient Boosting"""
    
    if model_path is None:
        model_files = glob(os.path.join(Config.MODEL_DIR, "gradientboosting_model_*.pkl"))
        if not model_files:
            print("❌ No Gradient Boosting models found!")
            return None
    ...
    if verbose:
        print(f"\n{'='*80}")
        print("🔍 GRADIENT BOOSTING INFERENCE ON SINGLE IMAGE")
        print(f"{'='*80}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Predict TB on single image using Gradient Boosting")
    parser.add_argument('--image', type=str, required=True, help='Path to X-ray image')
    parser.add_argument('--model', type=str, default=None, help='Path to GB model')
    
    args = parser.parse_args()
    
    result = infer_gb_on_single_image(args.image, args.model, verbose=True)
