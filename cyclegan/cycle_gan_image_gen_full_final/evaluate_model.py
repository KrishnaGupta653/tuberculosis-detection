...
def compute_ssim(img1, img2):
    ...
    # For grayscale images (2D), don't specify channel_axis
    channel_axis = None if len(img1.shape) == 2 else 2
...
def compute_contrast_preservation(img1, img2):
    ...
    contrast1 = np.sqrt(np.maximum(uniform_filter(img1.astype(float)**2, size=5) - mean1**2, 0))
    contrast2 = np.sqrt(np.maximum(uniform_filter(img2.astype(float)**2, size=5) - mean2**2, 0))
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.output, f"{results['image']}_evaluation.json"), 'w') as f:
        ...
        results_json = {
            ...
            'plausibility': {
                'mean_consistency': float(results['plausibility']['mean_consistency']),
                'extreme_pixels_ratio': float(results['plausibility']['extreme_pixels_ratio']),
                'is_plausible': bool(results['plausibility']['is_plausible'])
            }
        }
